...
> A portfolio of completed machine learning projects spanning data exploration, predictive modeling, binary classification, and LLM-powered chatbot development.

![Projects Completed](https://img.shields.io/badge/Projects%20Completed-5%20%2F%206-4ade80?style=flat-square&labelColor=0b0d11)
...
| 04 | General Health Query Chatbot — LLM & Prompt Engineering | — | LLaMA 3.1 via Groq API | ✅ Complete |
...
├── health care task/
│   ├── main (1).py            # Health Chatbot — Groq / LLaMA 3.1
│   ├── main.py                # Health Chatbot — HuggingFace / Mistral-7B
│   ├── requirements.txt
...
## 💬 Project 04 — General Health Query Chatbot

**Objective:** Build a chatbot that answers general health questions using a Large Language Model with engineered safety filters.

**Tools:** Groq API (LLaMA 3.1 8B Instant) · Hugging Face Inference API (Mistral-7B fallback)

**What was done:**
- Built two complete implementations — Groq API (primary) and Hugging Face serverless inference (fallback)
- Engineered a detailed system prompt with 6 hard safety rules — no diagnoses, no treatment plans, always defer to doctors
- Emergency keyword detector (chest pain, stroke, overdose, etc.) that bypasses the LLM for instant response
- Interactive CLI chat loop with example query runner included in both versions

**Prompt Engineering Highlights:**
```
"You are a helpful, empathetic, and informative general medical assistant.
 CRITICAL SAFETY RULES:
 1. Never provide specific diagnoses or personalized treatment plans.
 2. Always advise consulting a qualified healthcare professional.
 3. For emergencies — instruct the user to call emergency services immediately."
```

**Key Findings:**
- Keyword-based emergency filter is fast and reliable — no LLM round-trip for life-threatening queries
- LLaMA 3.1 via Groq produces significantly cleaner responses than raw HuggingFace inference
- Prompt engineering alone (without fine-tuning) is sufficient for safe general health Q&A

**Files:** `health care task/main (1).py` · `health care task/main.py`
...
# Project 04 — Chatbot (Groq version)
pip install groq python-dotenv

# Project 04 — Chatbot (HuggingFace version)
pip install huggingface-hub
```

Create a `.env` file inside the `health care task/` folder:
...
| **LLM & NLP** | Prompt engineering, system prompt design, safety filter implementation |
| **API Integration** | yfinance, Groq API, Hugging Face Inference API |
